...
export default async function getUser(userID: string): Promise<UserData|null> {
    ...
    if (x.status === 200) {
        ...
        // const itineraryIDs: string[] = Array.from(new Set(js.itineraries.L.map((i: any) => i.S)));
        // const itineraries: any[] = [];
        //
        // for (let itineraryID of itineraryIDs) {
        //     const itinerary = await (await import("./getItinerary")).default(itineraryID);
        //     if (itinerary) {
